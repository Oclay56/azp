...
2. For Same Game Multi requests, call `getStakeUiSgmBoard` before choosing finalists. This is the UI-truth board and overrides feed-only lines. Every returned row has a snapshot-backed `selectionToken` and a `rowId`; preserve the exact `selectionToken` first.
...
5. Only evaluate props that appear in the returned Stake-backed rows. For SGM, only evaluate props that appear in `getStakeUiSgmBoard`, and use the returned `selectionToken` when building review slips. Use `rowId` only as fallback.
...
- For multi-game SGM review slips, gather each game's exact UI-backed SGM rows first, then call `buildStakeUiReviewSlipBatch` once with `selectionTokens` copied from those rows. Do not call one single-game review-slip action per game unless the user explicitly wants separate slips.
- Never reconstruct an SGM build request from player name, odds, or line text when a `selectionToken` is available. The `selectionToken` is the clickable identity. Use `rowId` only as fallback.
...
2. Use only the returned UI-backed Same Game Multi rows for player, team, market, line, side, odds, `selectionToken`, and `rowId`.
...
5. If building a visible review slip, call `buildStakeUiReviewSlip` with the selected rows' exact `selectionTokens`.
...
2. For each requested game, call `getStakeUiSgmBoard` and use only UI-backed SGM rows with exact `selectionTokens`.
3. Pull MLB context for finalists where supported.
4. Choose the legs yourself.
5. Call `buildStakeUiReviewSlipBatch` once with every game's selected `selectionTokens` so the local helper uses one shared Stake page/slip.
